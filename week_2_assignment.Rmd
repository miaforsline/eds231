...
# Visualize the most common words in the first paragraph of each article 
```{r plot_frequencies}
tokenized %>%
...
Since this visual is not very helpful, we can incoroporate stop words to help filter out less meaningful words from our visual 
...
This visual is still too cluttered and can be further cleaned 
...
# Visualize 
However, it is still pretty illegible 
```{r cleaning, eval=FALSE}
...
- limit the graph to only include words that occur at least X times 
```{r cleaning, eval=FALSE}
