...
ggplot(data = sentiment_df, aes(x = date, color = polarity)) + 
  ...
  labs(y = "Developed Media Sentiment (no. headlines)",
       x = "Date",
       color = "Sentiment",
       title = "IPCC Nexis Uni Sentiment from April 4 - 11") + 
...
#unnest to word-level tokens, remove stop words, and join sentiment words
 ...
 #head(sent_words) 
 #note that "falls in love" is categorized as positive for "love" but negative for "falls" 
 #bigrams = 2 words
 #trigrams = 3 words 
 #but there's no database for bi/trigrams 
```
...
sent_scores <- sent_words %>%
  ...
  spread(sentiment, n) %>%
  mutate(raw_score = positive - negative, #single sentiment score per page = combine pos + neg 
  offset = mean(positive - negative), #what is the average sentiment per page? -3 
...
#dotted line = avg sentiment per page for the book 
#red = offset score
#purple = raw score 
```
