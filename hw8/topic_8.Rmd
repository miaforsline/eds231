...
```{r recipe}
#create recipe
#we want to predict the fatal incident reports based on the text using our training data 
...
```{r pre-process}
#we can add to the recipe 
#familiar pre-processing steps such as tokenizing, breaking down to the word level
recipe <- incidents_rec %>%
  step_tokenize(Text) %>%
  step_tokenfilter(Text, max_tokens = 1000) %>% #we want to keep the most frequent 1000 words 
  step_tfidf(Text) 
```
