...
# #count most frequent words across all reports 
# #tbl_df, tbl, data.frame 
# raw_words <- raw_text %>%
#   mutate(year = as.factor(year)) %>%
#   unnest_tokens(word, text) %>%
#   anti_join(add_stops, by = 'word') %>% #remove stop words 
#   count(year, word, sort = TRUE)
# 
# #count number of total words per report  
# total_words <- raw_words %>% 
#   group_by(year) %>% 
#   summarize(total = sum(n))
# 
# #dataframe with the frequncy of each word + the total number of words per report
...
```{r}
#zoom in on just one of our key terms
urban_cors <- word_cors %>% 
  filter(item1 == "urban") %>%
  mutate(n = 1:n())
urban_cors  %>%
