...
- Which terms are uniquely common to a document ? (Ex. "fy2017" is unique to the 2018 document)
- which terms are most unique and frequent in each document ? 

```{r}
report_tf_idf <- report_words %>%
  bind_tf_idf(word, year, n) %>% #for ex, "environmental" occurred 13,700 times.
  select(-total) %>%
  arrange(desc(tf_idf))
...
```
# tokens that are more than a single word (unigram) 
...
```{r convert_dfm}
toks2 <- tokens_ngrams(toks1, n=2) #tokenize at the 2-word level 
dfm2 <- dfm(toks2)
dfm2 <- dfm_remove(dfm2, pattern = c(stop_vec))

freq_words2 <- textstat_frequency(dfm2, n=20) #each feature is 2 words (more coherent than 1 word)
...
```{r}
keyness <- textstat_keyness(dfm2, target = 1) #target = 1 --> first document in the list, so in this case 2015
...
```
# Hierarchical clustering to assess document similarity 
